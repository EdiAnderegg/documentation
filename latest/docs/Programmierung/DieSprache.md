...
| Term | Beschreibung | Beispeil |
|:-----|:--------|:--------|
| Syntaktischen Fehler | Dies sind Fehler, welche eine Verletzung der "Grammatik" der Sprache aufweisen. | ```cs 
int number = "Hello"; // ❌ Error: Cannot implicitly convert type 'string' to 'int'
``` |
| Logischen Fehlern | - Der Code ist syntaktisch korrekt und die IDE (Integrated Development Environment) oder der Code-Editor melden keine Fehler. Erst wenn das Programm ausgeführt wird, liefert es unerwartete Ausgaben oder akzeptiert unerwartete Eingaben! | ```cs
using System;
